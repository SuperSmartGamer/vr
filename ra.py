...
def run_command(cmd, check_output=False, shell=False, sudo=False):
    ...
    try:
        if check_output:
            result = subprocess.run(full_cmd, check=True, capture_output=True, text=True, shell=shell, timeout=60) # Added timeout
            log_message(f"Command output:\n{result.stdout.strip()}")
            return result.stdout.strip()
        else:
            subprocess.run(full_cmd, check=True, shell=shell, timeout=60) # Added timeout
            return True
...
def install_tmate_system_wide():
    ...
    try:
        ...
        if tmate_executable_path and os.path.exists(tmate_executable_path):
            ...
            return False
    except (OSError, tarfile.ReadError, tarfile.FilterError) as e: # Catch specific tarfile errors
...
def configure_tmate_keybindings():
    ...
    try:
        ...
        escaped_content = TMATE_CONFIG_CONTENT.strip().replace('"', '\\"') # Escape quotes for shell=True
        write_cmd = f"echo \"{escaped_content}\" | sudo tee {TMATE_CONFIG_FILE}"
        if not run_command(write_cmd, shell=True):
            log_error(f"Failed to write tmate configuration file {TMATE_CONFIG_FILE}. Check sudo privileges or disk issues.")
            return False

        log_message(f"tmate configuration written to '{TMATE_CONFIG_FILE}'.")
        log_message("Note: Changes apply to new tmate sessions started by root. Existing sessions need to be restarted.")
        return True
    except Exception as e:
...
def get_current_tmate_session_info():
    ...
    try:
        # Use the explicit config file path for the root tmate session to ensure correct identification
        tmate_list_output = run_command(["tmate", "show-messages", "-f", TMATE_CONFIG_FILE], check_output=True)
        if tmate_list_output is False: # Command failed
        ...
        if session_links:
            log_message("Active tmate session found.")
            # Use a more specific pgrep to avoid false positives
            pid_output = run_command(["pgrep", "-f", f"tmate -S.*-f {TMATE_CONFIG_FILE}"], check_output=True)
            if pid_output:
                # pgrep might return multiple lines if multiple matching processes exist,
                # take the first one or ensure it's a single process for simplicity
                ...
                # Even if PID can't be found, we have links, so return them.
                return session_links, None
        else:
            log_message("Found tmate process output but could not extract valid session links.")
            return None, None
...
def start_new_tmate_session():
    ...
    Returns (links, process_id) or (None, None).
    """
    log_message("Starting a new tmate session...")
    try:
        # Popen to start tmate. The 'tmate' command implicitly finds its server
        ...
        process = subprocess.Popen(
            ...
            text=True,
            bufsize=1,
            env=os.environ.copy() # Inherit current environment to ensure tmate finds its libs/config
        )

        tmate_output_buffer = ""
        start_time = time.time()
        session_links = {}
        found_links_in_output = False

        while True:
            line = process.stdout.readline()
            if not line:
                # If process has finished (poll is not None) and no more output, break
                if process.poll() is not None:
                    break
                # Otherwise, give it a moment, still expecting output
                time.sleep(0.1)
                continue # Keep trying to read lines

            tmate_output_buffer += line
            log_message(f"New tmate session output: {line.strip()}")

            if "web session:" in line and "ssh session:" in line:
                found_links_in_output = True
                break

            if time.time() - start_time > 90: # Increased timeout for initial connection
                log_error("Timeout waiting for new tmate session links.")
                process.terminate()
        ...
        # Give tmate a little more time to settle even if links were found
        time.sleep(2)
        ...
        if process.poll() is not None:
            ...
            if stderr_output:
                log_error(f"tmate stderr: {stderr_output}")
            return None, None

        # Attempt to parse links from the buffer even if the loop didn't break on 'found_links_in_output'
        ...
        if not session_links:
            log_error("Could not find new tmate session links in output buffer.")
            log_error(f"Full new tmate session output buffer:\n{tmate_output_buffer}")
            # Try to kill any dangling tmate process if links not found
...
def write_shs_file(links):
    ...
    try:
        ...
        write_cmd = f"echo \"{content}\" | sudo tee {SHS_FILE_PATH}"
        if not run_command(write_cmd, shell=True):
            log_error(f"Failed to write to {SHS_FILE_PATH}. Check permissions or disk space.")
            return False

        log_message("shs.txt updated successfully.")
        return True
    except Exception as e:
...
def upload_urls_to_cloud():
    ...
    try:
        # Dynamically determine the path to upload.py within the service base directory
        # as it was copied there during installation
        target_upload_script_path = os.path.join(SERVICE_BASE_DIR, "upload.py")

        # Add SERVICE_BASE_DIR to sys.path temporarily to import upload.py
        sys.path.insert(0, SERVICE_BASE_DIR)
        
        # Check if upload.py actually exists before attempting import
        ...
        if hasattr(upload, 'upload_to_r2') and callable(upload.upload_to_r2):
            if upload.upload_to_r2(SHS_FILE_PATH):
                ...
                log_error("upload.upload_to_r2 function reported a failure.")
                return False
        else:
...
def run_monitor_loop():
    ...
    """
    log_message("tmate monitor service started.")
    tmate_pid = None
        ...
        try:
            ...
            if current_links and discovered_pid:
                if tmate_pid is None or tmate_pid != discovered_pid:
                    ...
                    log_message(f"tmate session (PID: {tmate_pid}) still active. No changes to links.")
                    # Periodically re-upload to ensure freshness, even if links haven't changed
                    upload_urls_to_cloud()
            else:
                log_warning("No active tmate session found or links could not be retrieved. Attempting to start a new one.")
                # Kill any lingering tmate processes associated with this service
                kill_cmd = f"pkill -f 'tmate -S.*-f {TMATE_CONFIG_FILE}'"
                run_command([kill_cmd], shell=True, sudo=True)
                time.sleep(2)
            ...
            log_error(f"An unhandled error occurred in the monitor loop: {e}", include_traceback=True)
            # Do not exit, just log and continue to the next iteration after delay
...
def install_systemd_service():
    ...
    # Copy the main script to the service directory
    if not run_command(["cp", current_script_path, target_script_path], sudo=True):
        log_error(f"Failed to copy main script to {SERVICE_BASE_DIR}.")
        return False
    ...
    # Dynamically determine and copy upload.py
    ...
    if os.path.exists(source_upload_script_path):
        ...
        if not run_command(["cp", source_upload_script_path, target_upload_script_path], sudo=True):
            log_error(f"Failed to copy upload.py to {SERVICE_BASE_DIR}.")
            return False
...
"""
    ...
    try:
        write_cmd = f"echo \"{service_content.strip()}\" | sudo tee {service_file_path}"
        if not run_command(write_cmd, shell=True):
            log_error(f"Failed to write service file to {service_file_path}.")
            return False
    except Exception as e:
        log_error(f"Error creating systemd service file: {e}", include_traceback=True)
        return False
